...
public interface IPhotosApiClient
{
    ...
    /// <summary>
    /// Редактирует описание (подпись) указанной фотографии (метод OK <c>photos.editPhoto</c>).
    /// </summary>
    /// <param name="photoId">Идентификатор редактируемой фотографии (параметр <c>photo_id</c>).</param>
    /// <param name="description">
    /// Новый текст описания (параметр <c>description</c>). Не может быть пустым: по документации OK
    /// отсутствующий параметр удаляет существующее описание, поэтому клиент намеренно отклоняет пустую
    /// строку (<see cref="ArgumentException"/>), чтобы случайно не стереть подпись.
    /// </param>
    /// <param name="context">
    /// Контекст запроса. Для фотографии группы используйте <c>GroupRequestContext</c> — он добавляет
    /// параметр <c>gid</c>, без которого правка группового фото не применяется.
    /// </param>
    /// <param name="cancellationToken">Токен отмены операции.</param>
    /// <returns>
    /// Boolean-ответ метода OK: <c>true</c> — описание обновлено; <c>false</c> — OK принял вызов,
    /// но правку НЕ применил (текст остался прежним). Ответ <c>false</c> приходит без ошибки,
    /// поэтому его обязательно нужно проверять — иначе отказ выглядит как успех.
    /// </returns>
    /// <remarks>
    /// По документации apiok.ru: сессия обязательна, требуемые права — <c>PHOTO_CONTENT</c> и
    /// <c>VALUABLE_ACCESS</c>.
    /// <list type="bullet">
    /// <item><description>Для фото <b>групп</b> (с <c>gid</c>) правка применяется штатно.</description></item>
    /// <item><description>Для фото <b>страниц</b> (профилей) OK на практике систематически возвращает
    /// <c>false</c>, не меняя текст, — планируйте запасной путь (например, перезаливку фото).</description></item>
    /// <item><description>При попытке редактирования чужой фотографии без прав метод также возвращает <c>false</c>.</description></item>
    /// </list>
    /// </remarks>
